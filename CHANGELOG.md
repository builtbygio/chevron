...
- Windows userData migration implemented and landed **inert** (H3 PR 23b, step 2 of [docs/windows-userdata-migrate.md](docs/windows-userdata-migrate.md)). `src/main-process/migrate-windows-userdata.js` copies config, trusted projects, StateStore and packages out of `%LOCALAPPDATA%\atom`, skipping regenerable caches, never overwriting existing destination data, writing a one-shot marker even on partial failure, and failing open. Runs in main before the first `BrowserWindow`. Requires `CHEVRON_USERDATA_MIGRATE=1`; `CHEVRON_SKIP_USERDATA_MIGRATE=1` disables it outright. **The name flip has not happened** — `generate-metadata.js` still writes the Atom-era Windows name, and a test asserts that, since the plan requires the migration to ship before the irreversible half.
