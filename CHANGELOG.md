...
- **#62 Option 2 — drop CoffeeScript runtime:** remove `coffee-script` from app dependencies; bootstrap decaffeinates the four bundled packages that still shipped `lib/*.coffee` (`autocomplete-atom-api`, `autocomplete-css`, `bookmarks`, `wrap-guide`); compile-cache refuses `.coffee` with a migration error; cpm warns on install if a package ships runtime Coffee. Babel 5 remains for prefix JS until a later cut. See [docs/babel-coffee-isolation-plan.md](docs/babel-coffee-isolation-plan.md).
