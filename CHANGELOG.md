...
- **Build modernization (Streams A–C + D policy):** host Node **20–24** / Python **3.11–3.13** gate; critical native rebuild **hard-fail** (override `CHEVRON_ALLOW_NATIVE_REBUILD_FAILURES=1`); [bootstrap-patch-matrix.md](docs/bootstrap-patch-matrix.md); silent no-op Coffee/Babel build steps; Electron ≥43 custom snapshot remains **opt-in** (`CHEVRON_FORCE_MKSNAPSHOT=1`). See [docs/build-modernization.md](docs/build-modernization.md).
