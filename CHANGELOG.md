...
- **Auto-update through GitHub Releases.** Atom updated through GitHub's Squirrel service, which no longer exists, so Chevron could only open the Releases page. Updates now go through electron-updater: every four hours (and on Check for Update) the app reads the latest release's metadata, downloads the new build in the background, and installs it on "Restart and Install Update" or on quit. On Windows that is an NSIS installer, which replaces the Squirrel one; on macOS it is the universal zip, installed by Squirrel.Mac, which requires a signed build, so an unsigned preview keeps opening the download page; Linux installs stay with their package manager and the download page. New `core.allowPrereleaseUpdates` opts a stable build into releases marked pre-release. `docs/reference/auto-update.md`.

- **Signed and notarized builds when the secrets exist.** macOS signs with a Developer ID from `CSC_LINK` and notarizes with notarytool from `APPLE_ID` / `APPLE_APP_SPECIFIC_PASSWORD` / `APPLE_TEAM_ID`; Windows signs the executables and the installer from `WIN_CSC_LINK`. The old scripts drove `altool`, retired by Apple in 2023, and passed option names the current `@electron/osx-sign` does not accept. Without the secrets every step skips, saying so, and the unsigned preview builds as before.
...
- **One macOS bundle for Intel and Apple Silicon.** The release now ships a universal `chevron-mac-universal.zip` instead of `chevron-mac-x64.zip` and `chevron-mac-arm64.zip`. The two builds are still made on their own hosts and merged afterwards with `lipo` (`./script/mac-universal`, `@electron/universal`); a new CI job does the merge, smoke-tests the result and uploads it, and the release job drops the per-arch zips. Packaging no longer ships node-gyp's static archives and stamp files (`build/Release/*.a` and friends), and the module cache no longer lists per-arch `prebuilds/` natives and is written in sorted order: nothing loads through those paths, and readdir order on APFS is not stable, so the two builds' `package.json` differed for no reason. `docs/reference/packaging.md`.
...
- Four `script/build` steps that had no inputs left, verified before removal: `transpile-cson-paths` (Wave 1 proved **0** `.cson` across all 94 pins and the app tree), `transpile-peg-js-paths` (**0** `.pegjs` sources), `transpile-packages-with-custom-transpiler-paths` (**0** packages declare `atomTranspilers`), and `generate-api-docs` (wrote an untracked `docs/output/atom-api.json` that nothing consumed once `script/vsts/` was deleted).
- The CoffeeScript build toolchain: `donna`, `joanna`, `tello` and `coffeelint`. `donna` is written in CoffeeScript, so this removes the last CoffeeScript from the build. `coffeelint` globbed three paths that are all empty, and `script/lint` never ran in GitHub CI.
- `script/ci/script-fossils.test.js`. It existed to assert that these fossils must **stay**, each on a premise that has now expired. Rewriting it to assert they are *gone* would have left a test whose only job is to record that nothing happens — upkeep with no payoff. The reasoning lives in this changelog and in `docs/decisions/`; `script/test` needs no guard because the nightly Jasmine workflow invokes it directly and would fail on its own.
- The chromedriver harness: `electron-chromedriver`, `check-chromedriver-version` and `spec/integration/` — macOS-only, run by nothing, and superseded by `script/ci/smoke-test.js`.
...
- The last 3 mismatched forks are reconciled: `fs-admin`, `git-utils` and `node-keytar` reported *ahead* of their pin but were **pristine upstream Atom** — zero Chevron references, HEAD on Atom's `add sunset message` — while their published packages carry Chevron's context-aware native registration. Publishing from any of them would have shipped an unmodified upstream native. Their versions were reconciled downwards (0.20.0 → 0.15.0, 5.7.3 → 5.7.1, 7.9.0 → 4.13.0); the upstream history remains in git. `script/audit-fork-drift.js` now warns that a higher repo version does not mean the repo is further along. **All 83 owned forks now match what they ship.**
