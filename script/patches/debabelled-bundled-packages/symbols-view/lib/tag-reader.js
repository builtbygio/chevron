...
var tag_reader_default = {
  find(editor, callback) {
    ...
    if (!symbols.length) {
      ...
      const wordRegExp = rubyScopes.length ? (nonWordCharacters = atom.config.get("editor.nonWordCharacters", { scope }), // Allow special handling for fully-qualified ruby constants
    ...
    import_async.default.map(atom.project.getPaths(), (projectPath, done) => {
  ...
  getAllTags(callback) {
    const projectTags = [];
    const task = import_atom.Task.once(handlerPath, atom.project.getPaths(), () => callback(projectTags));
    task.on("tags", (tags) => {
