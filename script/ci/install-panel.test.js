...
describe('the catalog ships as installable payloads', () => {
  ...
  it('they are unpacked from the asar', () => {
    // cpm installs by copying a directory and is a separate process, so it
    // cannot opendir inside the archive. Left packed, Install fails with
    // ENOTDIR while appearing to half-succeed.
    const policy = fs.readFileSync(
      path.join(ROOT, 'script', 'lib', 'packaging-policy.js'),
      'utf8'
    );
    assert.match(policy, /'catalog'/);
  });

  it('the panel prefers the unpacked copy', () => {
    const src = fs.readFileSync(path.join(LIB, 'install-panel.js'), 'utf8');
    assert.match(src, /\.asar/);
    assert.match(src, /unpacked/);
  });
...
describe('the panel is registered', () => {
  ...
  it('reports failure rather than leaving the button spinning', () => {
    ...
    // Node writes deprecation warnings to stderr ahead of anything cpm says,
    // and reporting the first line surfaced those instead of the real cause.
    assert.match(src, /DeprecationWarning/);
