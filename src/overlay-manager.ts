...
(OverlayManager = class OverlayManager {
  ...
  render(state) {
    for (var decorationId in state.content.overlays) {
      var overlay = state.content.overlays[decorationId];
    ...
    return (() => {
      const result = [];
      for (var id in this.overlaysById) {
        var {overlayNode} = this.overlaysById[id];
