...
2. **Wave 2 — complete.** Both items were owned-npm work, published under `@builtbygio`:  
   ...
   ⚠️ **Fork repos lag npm — 29 of 83, systemic.** Work gets published from a throwaway clone and never pushed back, so the "pin source" repo does not describe what ships, and publishing from it silently reverts the gap. `tree-view` was three releases *and* a whole `.js`→`.ts` conversion behind.  
   Run **`node script/audit-fork-drift.js`** before publishing any fork. It splits **behind** (26 — the dangerous kind) from **ahead** (3: `fs-admin` 0.20.0 vs pinned 0.15.0, `git-utils` 5.7.3 vs 5.7.1, `node-keytar` 7.9.0 vs 4.13.0 — repo work that was never published or never pinned; decide before reconciling).  
   Reconciled so far: `github`, `spell-check`, `image-view`, `snippets`, `tree-view`. **26 still lie.**  
