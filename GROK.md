...
2. **Wave 2 — complete.** Both items were owned-npm work, published under `@builtbygio`:  
   ...
   ✅ **Fork drift: reconciled.** Work used to be published from a throwaway clone and never pushed back, so a "pin source" repo described less than what shipped and publishing from it silently reverted the gap. An audit found **29 of 83**; all 26 *behind* are now reconciled to their published tarball, each proven by packing to exactly the published file set. `tree-view` was the worst — three releases and a whole `.js`→`.ts` conversion.  
   Run **`node script/audit-fork-drift.js`** before publishing any fork. **3 remain, all *ahead*** — `fs-admin` 0.20.0 vs pinned 0.15.0, `git-utils` 5.7.3 vs 5.7.1, `node-keytar` 7.9.0 vs 4.13.0. Those carry repo work never published or never pinned; that needs a decision (publish it, or drop it), not a reconcile.  
