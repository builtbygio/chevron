...
2. **Architecture modernization** — target + plan: [docs/chevron-architecture-modernization.md](docs/chevron-architecture-modernization.md). H1 (PR 1–12 + 2b) landed. H2 PR 13 catalog landed. PR 13b first port is YAML (this change). Next 13b is xml, then php/toml/sql — only if asked. Do **not** delete `Task` / `season` / `document-register-element` / first-mate. github epic waits on Q1 after dogfood.  
