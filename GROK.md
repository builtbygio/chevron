...
2. **Architecture modernization** — target + plan: [docs/chevron-architecture-modernization.md](docs/chevron-architecture-modernization.md). H1 (PR 1–12 + 2b) landed. H2 PR 13 catalog landed. PR 13b later tranche done. PR 13c: source/hyperlink/text landed; next `language-todo`. PR 15 landed (`@vscode/ripgrep@1.15.14`). PR 16 (this change): TypeScript-first for new `src/` files; existing `.js` grandfathered. PR 14 + 14a open as #160. Do **not** delete `Task` / `season` / `document-register-element` / first-mate. github epic waits on Q1 after dogfood.  
