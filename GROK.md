...
2. **Architecture modernization** — target + plan: [docs/chevron-architecture-modernization.md](docs/chevron-architecture-modernization.md). H1 PR 1 (docs) and PR 2 (ripgrep default) landed. Next: `rg` spawn from main (PR 2b), JSON config, `@electron/packager`, … Do **not** delete `Task` / `season` / `document-register-element` in H1. github epic waits on Q1 after dogfood.  
