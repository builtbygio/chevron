...
2. **Architecture modernization** — target + plan: [docs/chevron-architecture-modernization.md](docs/chevron-architecture-modernization.md). H1 PR 1–3 and PR 2b (`rg` from main) landed. Next: JSON config, `@electron/packager`, delete scandal search (PR 4), … Do **not** delete `Task` / `season` / `document-register-element` in H1. github epic waits on Q1 after dogfood.  
