...
2. **Architecture modernization** — target + plan: [docs/chevron-architecture-modernization.md](docs/chevron-architecture-modernization.md). H1–H2 through PR 17 + 14/14a landed. Next 13c is `language-gfm`. `Workspace.replace` still uses `Task`. Do **not** delete `Task` / `season` / `document-register-element` / first-mate. **Q1 is 8B.** Keep the inbox; skip Epic 18 / PR 19. `github` 0.37.9: React 18.3; graphql-client; GitHub App device-flow login (set `github.oauthClientId`); classic PAT fallback. Register the App once (`docs/github-app.md` in the package).  
