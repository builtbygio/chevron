...
2. **Architecture modernization** — target + plan: [docs/chevron-architecture-modernization.md](docs/chevron-architecture-modernization.md). H1–H2 through PR 17 + 14/14a landed. Next 13c is `language-todo`. `Workspace.replace` still uses `Task`. Do **not** delete `Task` / `season` / `document-register-element` / first-mate. **Q1 is 8B.** Keep the inbox; skip Epic 18 / PR 19. `github` 0.37.4: React 18.3; remaining QueryRenderers (detail/reviews/decorations/create-dialog) off Relay; load-more/refetch/reaction mutations are stubs. Long-term login is a Chevron GitHub App; classic PAT is interim.  
