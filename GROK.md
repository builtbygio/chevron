...
2. **Wave 2 — in progress.** Both items need an `npm publish` under `@builtbygio`, so neither can land from this repo alone:  
   - **github GraphQL** — done and verified in the fork: [builtbygio/github#16](https://github.com/builtbygio/github/pull/16). 8B already replaced Relay with `graphql-client` + `GraphQLQuery`; the old layer was dead weight (`relay-network-layer-manager.js` requires `relay-runtime`, which is not a dependency in any version; 76 `__generated__` artifacts; `graphql@14` required by nothing; a 655 KB `schema.graphql` feeding relay-compiler). **−51,303 lines.** Kept `lib/relay-stub.js` (live 8B code) and `graphql/recovered/` (read at runtime). **Next: merge #16, publish `@builtbygio/github@0.37.13`, bump the pin + lockfile here.**  
   - **`natural` log4js patch** — blocked: `@builtbygio/natural` does not exist on npm. The patch is **load-bearing**, not cosmetic — `natural@0.4.0` declares `log4js: "*"`, which resolves to 6.9.1 where `logger.setLevel` is `undefined`, so unpatched `natural` throws at require time and takes the spell-check stack with it. Needs `@builtbygio/natural@0.4.1` published, then `@builtbygio/spell-check` repointed and republished, then the patch dropped here. Guarded by `script/ci/patch-inventory.test.js`.  
   ...
   ⚠️ **Fork repos lag npm.** `builtbygio/github` `main` was 0.37.9 while npm shipped 0.37.12 (28 files: the `atom.`→`chevron.` conversion, published but never pushed). `spell-check` (repo 0.77.3 / npm 0.77.5) and `tree-view` (repo 0.229.3 / npm 0.229.6) match the pattern. #16 reconciles github; the others still lie.  
