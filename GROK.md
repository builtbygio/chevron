...
2. **Architecture modernization** — target + plan: [docs/chevron-architecture-modernization.md](docs/chevron-architecture-modernization.md). H1 PR 1–4 and PR 2b landed (docs, product rg, replace off scandal, rg from main, scandal search deleted). Next: JSON config (PR 5), `@electron/packager` (PR 6). Do **not** delete `Task` / `season` / `document-register-element` in H1. github epic waits on Q1 after dogfood.  
