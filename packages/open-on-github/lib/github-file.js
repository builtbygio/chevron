...
class GitHubFile {
  ...
  constructor(filePath) {
    ...
    const [rootDir] = chevron.project.relativizePath(this.filePath);
    if (rootDir != null) {
      const rootDirIndex = chevron.project.getPaths().indexOf(rootDir);
      this.repo = chevron.project.getRepositories()[rootDirIndex];
      this.type = "none";
  ...
  blame(lineRange) {
    if (this.validateRepo()) {
      if (this.type === "repo") {
        ...
        chevron.notifications.addWarning(`Blames do not exist for ${this.type}s`);
      }
    }
  }
  history() {
  ...
  copyURL(lineRange) {
    if (this.validateRepo()) {
      chevron.clipboard.write(this.shaURL() + this.getLineRangeSuffix(lineRange));
    }
  }
  openBranchCompare() {
    if (this.validateRepo()) {
      if (this.type === "repo") {
        ...
        chevron.notifications.addWarning(`Branches do not exist for ${this.type}s`);
      }
    }
  }
  openIssues() {
    if (this.validateRepo()) {
      if (this.type === "repo") {
        ...
        chevron.notifications.addWarning(`Issues do not exist for ${this.type}s`);
      }
    }
  }
  openPullRequests() {
    if (this.validateRepo()) {
      if (this.type === "repo") {
        ...
        chevron.notifications.addWarning(`Pull requests do not exist for ${this.type}s`);
      }
    }
  }
  openRepository() {
  ...
  getLineRangeSuffix(lineRange) {
    if (lineRange && this.type !== "wiki" && chevron.config.get("open-on-github.includeLineNumbersInUrls")) {
  ...
  validateRepo() {
    if (!this.repo) {
      chevron.notifications.addWarning(`No repository found for path: ${this.filePath}.`);
      return false;
    } else if (!this.gitURL()) {
      chevron.notifications.addWarning(`No URL defined for remote: ${this.remoteName()}`);
      return false;
    } else if (!this.githubRepoURL()) {
      chevron.notifications.addWarning(`Remote URL is not hosted on GitHub: ${this.gitURL()}`);
      return false;
    }
    return true;
  }
