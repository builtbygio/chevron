...
class BackgroundTipsElement {
  constructor () {
    ...
    this.index = -1
    this.workspaceCenter = chevron.workspace.getCenter()
    ...
    chevron.getCurrentWindow().on('blur', visibilityCallback)
    chevron.getCurrentWindow().on('focus', visibilityCallback)

    this.disposables.add(new Disposable(() => chevron.getCurrentWindow().removeListener('blur', visibilityCallback)))
    this.disposables.add(new Disposable(() => chevron.getCurrentWindow().removeListener('focus', visibilityCallback)))
  ...
  attach () {
    ...
    const paneView = chevron.views.getView(this.workspaceCenter.getActivePane())
  ...
  shouldBeAttached () {
    ...
    chevron.getCurrentWindow().isFocused()
  }
  ...
  renderTip (str) {
    str = str.replace(/\{(.+)\}/g, (match, command) => {
      ...
      const bindings = chevron.keymaps.findKeyBindings({command: command.trim()})
