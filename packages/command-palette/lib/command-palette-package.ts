...
class CommandPalettePackage {
  activate () {
    ...
    this.disposables.add(chevron.config.observe('command-palette.useAlternateScoring', (newValue) => {
      this.commandPaletteView.update({useAlternateScoring: newValue})
    }))
    this.disposables.add(chevron.config.observe('command-palette.preserveLastSearch', (newValue) => {
