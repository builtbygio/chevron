...
class CommandPaletteView {
  ...
  async show (showHiddenCommands = false) {
    ...
    this.activeElement = (document.activeElement === document.body) ? chevron.views.getView(chevron.workspace) : document.activeElement
    this.keyBindingsForActiveElement = chevron.keymaps.findKeyBindings({target: this.activeElement})
    const commandsForActiveElement = chevron.commands
