...
module.exports = {
  ...
  triggerAutocomplete(editor) {
    return chevron.commands.dispatch(chevron.views.getView(editor), 'autocomplete-plus:activate', {activatedManually: false});
  },
  ...
  getPreviousPropertyName(bufferPosition, editor) {
    ...
    while (row >= 0) {
      ...
      var propertyName = __guard__(inlinePropertyNameWithColonPattern.exec(line), x => x[1]);
  ...
  getPropertyNameCompletions({bufferPosition, editor, scopeDescriptor, activatedManually}) {
    ...
    for (var property in this.properties) {
      var options = this.properties[property];
  ...
  getTagCompletions({bufferPosition, editor, prefix}) {
    const completions = [];
    if (prefix) {
      for (var tag of Array.from(this.tags)) {
