...
const {TextEditor, BufferedProcess, CompositeDisposable, Disposable} = require('chevron')
...
class PackageGeneratorView {
  constructor () {
    ...
    this.disposables.add(chevron.commands.add('atom-workspace', {
  ...
  attach (mode) {
    this.mode = mode
    if (this.panel == null) this.panel = chevron.workspace.addModalPanel({item: this, visible: false})
  ...
  confirm () {
    if (this.validPackagePath()) {
      this.createPackageFiles(() => {
        ...
        chevron.open({pathsToOpen: [packagePath]})
        this.close()
      })
    }
  }
  ...
  getPackagesDirectory () {
    return process.env.ATOM_REPOS_HOME || chevron.config.get('core.projectHome') || path.join(fs.getHomeDirectory(), 'github')
  }
  ...
  getInitOptions (packagePath) {
    ...
    if (this.mode !== 'theme') {
      return [...options, '--syntax', chevron.config.get('package-generator.packageSyntax')]
    } else {
      return options
    }
  }
  ...
  initPackage (packagePath, callback) {
    ...
    this.runCommand(chevron.packages.getApmPath(), command, callback)
  }

  linkPackage (packagePath, callback) {
    const args = ['link']
    if (chevron.config.get('package-generator.createInDevMode')) args.push('--dev')
    args.push(packagePath.toString())

    this.runCommand(chevron.packages.getApmPath(), args, callback)
  }

  isStoredInDotAtom (packagePath) {
    const packagesPath = path.join(chevron.getConfigDirPath(), 'packages', path.sep)
    if (packagePath.startsWith(packagesPath)) return true

    const devPackagesPath = path.join(chevron.getConfigDirPath(), 'dev', 'packages', path.sep)
