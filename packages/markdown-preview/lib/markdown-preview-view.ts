...
module.exports = class MarkdownPreviewView {
  ...
  constructor ({ editorId, filePath }) {
    ...
    } else if (chevron.packages.hasActivatedInitialPackages()) {
      this.subscribeToFilePath(this.filePath)
    } else {
      this.disposables.add(
        chevron.packages.onDidActivateInitialPackages(() => {
          this.subscribeToFilePath(this.filePath)
        })
      )
    }
  ...
  resolveEditor (editorId) {
    ...
    if (chevron.packages.hasActivatedInitialPackages()) {
      ...
      this.disposables.add(chevron.packages.onDidActivateInitialPackages(resolve))
    }
  }
  ...
  handleEvents () {
    ...
    this.disposables.add(
      chevron.grammars.onDidAddGrammar(() => lazyRenderMarkdown())
    )
    if (typeof chevron.grammars.onDidRemoveGrammar === 'function') {
      this.disposables.add(
        chevron.grammars.onDidRemoveGrammar(() => lazyRenderMarkdown())
      )
    } else {
      // TODO: Remove onDidUpdateGrammar hook once onDidRemoveGrammar is released
      this.disposables.add(
        chevron.grammars.onDidUpdateGrammar(() => lazyRenderMarkdown())
      )
    }
    ...
    const changeHandler = () => {
      ...
      const pane = chevron.workspace.paneForItem(this)
      if (pane != null && pane !== chevron.workspace.getActivePane()) {
    ...
    if (this.file) {
      ...
      this.disposables.add(
        this.editor.getBuffer().onDidStopChanging(function () {
          if (chevron.config.get('markdown-preview.liveUpdate')) {
            changeHandler()
          }
        })
      )
    ...
    this.disposables.add(
      chevron.config.observe('markdown-preview.useGitHubStyle', useGitHubStyle => {
        if (useGitHubStyle) {
  ...
  getTextEditorStyles () {
    ...
    textEditorStyles.initialize(chevron.styles)
  ...
  async copyToClipboard () {
    ...
    if (
      ...
      chevron.clipboard.write(selectedText)
    } else {
      try {
        ...
        chevron.notifications.addError('Copying Markdown as HTML failed', {
          dismissable: true,
  ...
  getSaveDialogOptions () {
    ...
    if (defaultPath) {
      ...
      if ((projectPath = chevron.project.getPaths()[0])) {
...
<html>
  ...
  <body class='markdown-preview' data-use-github-style>${htmlBody}</body>
    ...
    return chevron.workspace.open(htmlFilePath)
  }
}
