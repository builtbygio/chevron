...
var render = function (text, filePath) {
  ...
  marked.setOptions({
    sanitize: false,
    breaks: chevron.config.get('markdown-preview.breakOnSingleNewline'),
    renderer
  })
  ...
  html = createDOMPurify().sanitize(html, {
    ALLOW_UNKNOWN_PROTOCOLS: chevron.config.get(
...
var resolveImagePaths = function (element, filePath) {
  const [rootDirectory] = chevron.project.relativizePath(filePath)
...
var highlightCodeBlocks = function (domFragment, grammar, editorCallback) {
  ...
  if ((fontFamily = chevron.config.get('editor.fontFamily'))) {
  ...
  for (const preElement of domFragment.querySelectorAll('pre')) {
    ...
    chevron.grammars.assignLanguageMode(editor, scopeForFenceName(fenceName))
