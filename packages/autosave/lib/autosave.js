...
module.exports = {
  ...
  activate () {
    ...
    this.subscriptions.add(chevron.workspace.onDidAddPaneItem(({item}) => this.autosavePaneItem(item, true)))
    this.subscriptions.add(chevron.workspace.onWillDestroyPaneItem(({item}) => this.autosavePaneItem(item)))
  },
  ...
  autosavePaneItem (paneItem, create = false) {
    if (!chevron.config.get('autosave.enabled')) return
    if (!paneItem) return
    ...
    const pane = chevron.workspace.paneForItem(paneItem)
  ...
  autosaveAllPaneItems () {
    return Promise.all(
      chevron.workspace.getPaneItems().map((paneItem) => this.autosavePaneItem(paneItem))
    )
  }
}
