...
module.exports = SpellCheckView = class SpellCheckView {
    constructor(editor, spellCheckModule, manager) {
        ...
        this.correctMisspellingCommand = chevron.commands.add(
            chevron.views.getView(this.editor),
        ...
        this.disposables.add(
            chevron.config.onDidChange('editor.fontSize', () => {
                return this.subscribeToBuffer();
            })
        );

        this.disposables.add(
            chevron.config.onDidChange('spell-check.grammars', () => {
                return this.subscribeToBuffer();
            })
        );
    ...
    spellCheckCurrentGrammar() {
        ...
        return _.contains(chevron.config.get('spell-check.grammars'), grammar);
    }
    ...
    getCorrections(marker) {
        ...
        if (this.buffer != null && this.buffer.file && this.buffer.file.path) {
            [projectPath, relativePath] = chevron.project.relativizePath(
    ...
    addContextMenuEntries(mouseEvent) {
        ...
        const currentScreenPosition = chevron.views
            .getView(this.editor)
        ...
        if (
            ...
            if (corrections.length > 0) {
                this.spellCheckModule.contextMenuEntries.push({
                    menuItem: chevron.contextMenu.add({
                ...
                for (let correction of corrections) {
                    ...
                    contextMenuEntry.menuItem = chevron.contextMenu.add({
                        'atom-text-editor': [
