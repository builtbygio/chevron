...
class LocaleChecker {
    ...
    deferredInit() {
        ...
        const noticesMode = chevron.config.get('spell-check.noticesMode');

        if (noticesMode === 'console' || noticesMode === 'both') {
            console.log(this.getId(), message + searches);
        }
        if (noticesMode === 'popup' || noticesMode === 'both') {
            return chevron.notifications.addWarning(message, {
                buttons: [
                    {
                        className: 'btn',
                        onDidClick() {
                            return chevron.workspace.open(
                                'chevron://config/packages/spell-check'
                            );
                        },
                        text: 'Settings',
                    },
