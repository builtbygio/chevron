...
class ProjectView extends FuzzyFinderView {
  constructor (paths, metricsReporter) {
    ...
    this.disposables.add(chevron.config.onDidChange('fuzzy-finder.ignoredNames', () => { this.reloadPaths = true }))
    this.disposables.add(chevron.config.onDidChange('core.followSymlinks', () => { this.reloadPaths = true }))
    this.disposables.add(chevron.config.onDidChange('core.ignoredNames', () => { this.reloadPaths = true }))
    this.disposables.add(chevron.config.onDidChange('core.excludeVcsIgnoredPaths', () => { this.reloadPaths = true }))
  ...
  async reloadPathsIfNeeded () {
    if (this.reloadPaths) {
      ...
      if (chevron.project.getPaths().length === 0) {
      ...
      try {
        ...
        if (error.code === 'ENOENT' || error.code === 'EPERM') {
          chevron.notifications.addError('Project path not found!', {detail: error.message})
        } else {
          throw error
        }
      }
