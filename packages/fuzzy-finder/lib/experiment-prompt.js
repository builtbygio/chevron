...
function showNotification (message, {description, confirmBtn, cancelBtn, confirmFn, cancelFn}) {
  const notification = chevron.notifications.addInfo(
    message,
    {
      description,
      dismissable: true,
  ...
  showNotification(
    'Introducing experimental fast mode',
    {
      ...
      confirmFn: () => {
        ...
        chevron.config.set('fuzzy-finder.useRipGrep', true)
        chevron.config.set('fuzzy-finder.scoringSystem', SCORING_SYSTEMS.FAST)
        chevron.notifications.addSuccess('Fasten your seatbelt! Here comes a faster fuzzy finder.', {icon: 'rocket'})
      },
  ...
  showNotification(
    'Do you want to disable the new experimental fast mode for the fuzzy finder?',
    {
      ...
      confirmFn: () => {
        ...
        chevron.config.set('fuzzy-finder.useRipGrep', false)
        chevron.config.set('fuzzy-finder.scoringSystem', SCORING_SYSTEMS.ALTERNATE)
        chevron.notifications.addSuccess('OK. Experimental fast mode is disabled.')
      },
      cancelBtn: 'No, thanks',
      cancelFn: () => {
...
function isFastModeEnabled () {
  return (
    chevron.config.get('fuzzy-finder.scoringSystem') === SCORING_SYSTEMS.FAST &&
    chevron.config.get('fuzzy-finder.useRipGrep') === true
  )
}
