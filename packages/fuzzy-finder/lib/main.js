...
module.exports = {
  activate (state) {
    ...
    for (let editor of chevron.workspace.getTextEditors()) {
      editor.lastOpened = state[editor.getPath()]
    }

    this.disposables.add(chevron.workspace.observePanes(pane => {
  ...
  startLoadPathsTask () {
    ...
    if (chevron.project.getPaths().length === 0) return

    const PathLoader = require('./path-loader')
    this.loadPathsTask = PathLoader.startTask((projectPaths) => {
      this.projectPaths = projectPaths
    }, metricsReporter)
    this.projectPathsSubscription = chevron.project.onDidChangePaths(() => {
      this.projectPaths = null
      this.stopLoadPathsTask()
    })
  },
