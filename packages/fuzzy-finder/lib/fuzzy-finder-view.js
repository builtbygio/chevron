const {Point, CompositeDisposable} = require('chevron')
...
module.exports = class FuzzyFinderView {
  constructor (metricsReporter) {
    ...
    this.selectListView = new SelectListView({
      ...
      didConfirmSelection: (item) => {
        this.confirm(item, {searchAllPanes: chevron.config.get('fuzzy-finder.searchAllPanes')})
      },
    ...
    chevron.commands.add(this.selectListView.element, {
      ...
      'fuzzy-finder:invert-confirm': () => {
        this.confirm(
          ...
          {searchAllPanes: !chevron.config.get('fuzzy-finder.searchAllPanes')}
        )
      }
    })
    ...
    this.subscriptions.add(
      chevron.config.observe('fuzzy-finder.scoringSystem', (newValue) => {
  ...
  cancel () {
    if (chevron.config.get('fuzzy-finder.preserveLastSearch')) {
  ...
  confirm ({uri} = {}, openOptions) {
    if (chevron.workspace.getActiveTextEditor() && this.isQueryALineJump()) {
      const caretPosition = this.getCaretPosition()
      this.cancel()
      this.moveToCaretPosition(caretPosition)
    } else if (!uri) {
      this.cancel()
    } else if (chevron.applicationDelegate.isDirectorySync(uri)) {
  ...
  show () {
    ...
    this.panel.show()
    if (chevron.config.get('fuzzy-finder.prefillFromSelection') === true) {
  ...
  async openURI (uri, caretPosition, openOptions) {
    if (uri) {
      await chevron.workspace.open(uri, openOptions)
      this.moveToCaretPosition(caretPosition)
    }
  }
  ...
  convertPathToSelectViewObject (filePath) {
    const projectHasMultipleDirectories = chevron.project.getDirectories().length > 1

    const [rootPath, projectRelativePath] = chevron.project.relativizePath(filePath)
    const label =
