...
class GithubPackage {
  constructor({
    ...
    this.configPath = _path.default.join(configDirPath, 'github.json');
    ...
    this.renderFn = renderFn || ((component, node, callback) => {
      return (0, _reactRoot.render)(component, node, callback);
  ...
  rerender(callback) {
    ...
    if (!this.element) {
      ...
      this.subscriptions.add(new _eventKit.Disposable(() => {
        (0, _reactRoot.unmount)(this.element);
