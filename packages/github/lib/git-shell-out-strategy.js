...
class GitShellOutStrategy {
  ...
  async exec(args, options = GitShellOutStrategy.defaultExecArgs) {
    ...
    const diagnosticsEnabled = process.env.ATOM_GITHUB_GIT_DIAGNOSTICS || chevron.config.get('github.gitDiagnostics');
    ...
    return this.commandQueue.push(async () => {
      ...
      if (useGitPromptServer) {
        ...
        env.ATOM_GITHUB_SPEC_MODE = chevron.inSpecMode() ? 'true' : 'false';
