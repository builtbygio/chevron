...
class DurationSet {
  ...
  mark(eventName) {
    ...
    if (chevron.config.get('github.performanceToDirectory') !== '') {
      this.marks.push({
        eventName,
        duration
      });
...
function shouldCapture(seriesName, eventName) {
  const anyActive = ['Console', 'Directory', 'Profile'].some(kind => {
    const value = chevron.config.get(`github.performanceTo${kind}`);
  ...
  const mask = new RegExp(chevron.config.get('github.performanceMask'));
...
const yardstick = {
  async save() {
    const destDir = chevron.config.get('github.performanceToDirectory');
