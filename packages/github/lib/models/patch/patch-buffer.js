...
class PatchBuffer {
  constructor() {
    this.buffer = new _chevron.TextBuffer();
    this.buffer.retain();
  ...
  createInserterAt(insertionPoint) {
    return new Inserter(this, _chevron.Point.fromObject(insertionPoint));
  }
  ...
  createSubBuffer(rangeLike, options = {}) {
    ...
    const range = _chevron.Range.fromObject(rangeLike);
  ...
  inspect(opts = {}) {
    ...
    let inspectPoint = _chevron.Point.fromObject([0, 0]);
...
class Inserter {
  ...
  markWhile(layerName, block, markerOpts) {
    ...
    this.markerBlueprints.push({
      layerName,
      range: new _chevron.Range(start, end),
      markerOpts
    });
    return this;
  }
