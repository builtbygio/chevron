...
class BookmarksView {
  constructor(editorsBookmarks) {
    ...
    this.selectList = new import_atom_select_list.default({
      ...
      didConfirmSelection: ({ editor, marker }) => {
        ...
        chevron.workspace.paneForItem(editor).activate();
        chevron.workspace.paneForItem(editor).activateItem(editor);
      },
      didCancelSelection: () => {
        this.hide();
      },
  ...
  getModalPanel() {
    if (!this.modalPanel) {
      this.modalPanel = chevron.workspace.addModalPanel({ item: this.selectList });
    }
