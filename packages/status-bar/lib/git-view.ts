...
const {CompositeDisposable, GitRepositoryAsync} = require("chevron");
...
(GitView = class GitView {
  constructor() {
    ...
    this.activeItemSubscription = chevron.workspace.getCenter().onDidChangeActivePaneItem(() => {
      return this.subscribeToActiveItem();
    });
    this.projectPathSubscription = chevron.project.onDidChangePaths(() => {
  ...
  subscribeToRepositories() {
    ...
    return (() => {
      const result = [];
      for (var repo of Array.from(chevron.project.getRepositories())) {
        if (repo != null) {
  ...
  getRepositoryForActiveItem() {
    const [rootDir] = Array.from(chevron.project.relativizePath(this.getActiveItemPath()));
    const rootDirIndex = chevron.project.getPaths().indexOf(rootDir);
    if (rootDirIndex >= 0) {
      return chevron.project.getRepositories()[rootDirIndex];
    } else {
      for (var repo of Array.from(chevron.project.getRepositories())) {
        if (repo) {
          return repo;
  ...
  getActiveItem() {
    return chevron.workspace.getCenter().getActivePaneItem();
  }
  ...
  updateBranchText(repo) {
    if (this.showGitInformation(repo)) {
      ...
      return this.branchTooltipDisposable = chevron.tooltips.add(this.branchArea, {title: `On branch ${head}`});
    } else {
  ...
  showGitInformation(repo) {
    ...
    if ((itemPath = this.getActiveItemPath())) {
      return chevron.project.contains(itemPath);
    } else {
  ...
  updateAheadBehindCount(repo) {
    ...
    if (ahead > 0) {
      ...
      this.commitsAheadTooltipDisposable = chevron.tooltips.add(this.commitsAhead, {title: `${_.pluralize(ahead, 'commit')} ahead of upstream`});
    } else {
    ...
    if (behind > 0) {
      ...
      this.commitsBehindTooltipDisposable = chevron.tooltips.add(this.commitsBehind, {title: `${_.pluralize(behind, 'commit')} behind upstream`});
    } else {
  ...
  updateAsNewFile() {
    ...
    if (textEditor = chevron.workspace.getActiveTextEditor()) {
  ...
  updateTooltipText(text) {
    ...
    if (text) {
      return this.statusTooltipDisposable = chevron.tooltips.add(this.gitStatusIcon, {title: text});
    }
  }
