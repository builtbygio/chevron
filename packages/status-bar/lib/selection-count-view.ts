...
(SelectionCountView = class SelectionCountView {
  constructor() {
    ...
    this.tooltipDisposable = chevron.tooltips.add(this.element, {item: this.tooltipElement});

    this.formatString = (left = chevron.config.get('status-bar.selectionCountFormat')) != null ? left : '(%L, %C)';

    this.activeItemSubscription = chevron.workspace.onDidChangeActiveTextEditor(() => this.subscribeToActiveTextEditor());
  ...
  subscribeToConfig() {
    ...
    return this.configSubscription = chevron.config.observe('status-bar.selectionCountFormat', value => {
  ...
  scheduleUpdateCount() {
    if (!this.scheduledUpdate) {
      ...
      return chevron.views.updateDocument(() => {
        this.updateCount();
