...
(FileInfoView = class FileInfoView {
  ...
  registerTooltip() {
    return this.tooltip = chevron.tooltips.add(this.element, { title() {
  ...
  showCopiedTooltip(copyRelativePath) {
    ...
    return this.copiedTooltip = chevron.tooltips.add(this.element, {
      title: `Copied: ${text}`,
      trigger: 'manual',
      delay: {
        show: 0
  ...
  getActiveItemCopyText(copyRelativePath) {
    ...
    if (copyRelativePath) {
      const relativized = chevron.project.relativize(path);
      if (relativized !== path) {
        return relativized;
      }
    }
