...
(CursorPositionView = class CursorPositionView {
  constructor() {
    ...
    this.formatString = (left = chevron.config.get('status-bar.cursorPositionFormat')) != null ? left : '%L:%C';

    this.activeItemSubscription = chevron.workspace.onDidChangeActiveTextEditor(activeEditor => this.subscribeToActiveTextEditor());

    this.subscribeToConfig();
    this.subscribeToActiveTextEditor();

    this.tooltip = chevron.tooltips.add(this.element, {title: () => `Line ${this.row}, Column ${this.column}`});
  ...
  subscribeToActiveTextEditor() {
    ...
    const selectionsMarkerLayer = __guard__(chevron.workspace.getActiveTextEditor(), x => x.selectionsMarkerLayer);
  ...
  subscribeToConfig() {
    ...
    return this.configSubscription = chevron.config.observe('status-bar.cursorPositionFormat', value => {
  ...
  handleClick() {
    const clickHandler = () => chevron.commands.dispatch(chevron.views.getView(chevron.workspace.getActiveTextEditor()), 'go-to-line:toggle');
  ...
  scheduleUpdate() {
    ...
    return this.updateSubscription = chevron.views.updateDocument(() => {
      let position;
      this.viewUpdatePending = false;
      if (position = __guard__(chevron.workspace.getActiveTextEditor(), x => x.getCursorBufferPosition())) {
