...
(LaunchModeView = class LaunchModeView {
  constructor(param) {
    ...
    if (devMode) {
      ...
      this.tooltipDisposable = chevron.tooltips.add(this.element, {title: 'This window is in dev mode'});
    } else if (safeMode) {
      this.element.classList.add('text-success');
      this.tooltipDisposable = chevron.tooltips.add(this.element, {title: 'This window is in safe mode'});
    }
  }
