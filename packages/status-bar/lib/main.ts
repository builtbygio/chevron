...
module.exports = {
  activate() {
    ...
    this.subscriptions.add(chevron.config.onDidChange('status-bar.fullWidth', () => {
    ...
    this.statusBarVisibilitySubscription =
      chevron.config.observe('status-bar.isVisible', () => {
        return this.updateStatusBarVisibility();
      });

    chevron.commands.add('atom-workspace', 'status-bar:toggle', () => {
      if (this.statusBarPanel.isVisible()) {
        return chevron.config.set('status-bar.isVisible', false);
      } else {
        return chevron.config.set('status-bar.isVisible', true);
      }
    });

    const {safeMode, devMode} = chevron.getLoadSettings();
    if (safeMode || devMode) {
  ...
  deactivate() {
    ...
    if (chevron.__workspaceView != null) { return delete chevron.__workspaceView.statusBar; }
  },

  updateStatusBarVisibility() {
    if (chevron.config.get('status-bar.isVisible')) {
  ...
  attachStatusBar() {
    ...
    if (chevron.config.get('status-bar.fullWidth')) {
      return this.statusBarPanel = chevron.workspace.addFooterPanel(panelArgs);
    } else {
      return this.statusBarPanel = chevron.workspace.addBottomPanel(panelArgs);
    }
  },
