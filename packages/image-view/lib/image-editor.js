...
const {Emitter, File, CompositeDisposable} = require('chevron')
...
class ImageEditor {
  ...
  constructor (filePath) {
    ...
    this.subscriptions.add(this.file.onDidDelete(() => {
      const pane = chevron.workspace.paneForURI(filePath)
      try {
  ...
  terminatePendingState () {
    if (this.isEqual(chevron.workspace.getCenter().getActivePane().getPendingItem())) {
