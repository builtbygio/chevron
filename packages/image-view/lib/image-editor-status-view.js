...
class ImageEditorStatusView {
  constructor (statusBar) {
    ...
    this.disposables.add(chevron.workspace.getCenter().onDidChangeActivePaneItem(() => { this.updateImageSize() }))
  }
  ...
  updateImageSize () {
    ...
    if (editor instanceof ImageEditor && editor.view instanceof ImageEditorView) {
      ...
      const callback = () => {
        if (editor === chevron.workspace.getCenter().getActivePaneItem()) {
          this.getImageSize(this.editorView)
        }
      }
