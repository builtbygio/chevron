...
class KeyBindingResolverView {
  constructor() {
    ...
    this.disposables.add(chevron.workspace.getBottomDock().observeActivePaneItem((item) => {
      if (item === this) {
        this.attach();
      } else {
        this.detach();
    ...
    this.disposables.add(chevron.workspace.getBottomDock().observeVisible((visible) => {
      if (visible) {
        if (chevron.workspace.getBottomDock().getActivePaneItem() === this) this.attach();
      } else {
        this.detach();
      }
    }));
  ...
  getURI() {
    return "chevron://keybinding-resolver";
  }
  serialize() {
    return {
  ...
  attach() {
    ...
    this.attached = true;
    this.keybindingDisposables = new import_atom.CompositeDisposable();
    this.keybindingDisposables.add(chevron.keymaps.onDidMatchBinding(({ keystrokes, binding, keyboardEventTarget, eventType }) => {
      if (eventType === "keyup" && binding == null) {
        return;
      }
      const unusedKeyBindings = chevron.keymaps.findKeyBindings({ keystrokes, target: keyboardEventTarget }).filter((b) => b !== binding);
      const unmatchedKeyBindings = chevron.keymaps.findKeyBindings({ keystrokes }).filter((b) => b !== binding && !unusedKeyBindings.includes(b));
      this.update({ usedKeyBinding: binding, unusedKeyBindings, unmatchedKeyBindings, keystrokes });
    }));
    this.keybindingDisposables.add(chevron.keymaps.onDidPartiallyMatchBindings(({ keystrokes, partiallyMatchedBindings }) => {
      this.update({ keystrokes, partiallyMatchedBindings });
    }));
    this.keybindingDisposables.add(chevron.keymaps.onDidFailToMatchBinding(({ keystrokes, keyboardEventTarget, eventType }) => {
      if (eventType === "keyup") {
        return;
      }
      const unusedKeyBindings = chevron.keymaps.findKeyBindings({ keystrokes, target: keyboardEventTarget });
      const unmatchedKeyBindings = chevron.keymaps.findKeyBindings({ keystrokes }).filter((b) => !unusedKeyBindings.includes(b));
  ...
  extractBundledKeymap(bundledKeymapPath) {
    const metadata = require(import_path.default.join(chevron.getLoadSettings().resourcePath, "package.json"));
  ...
  extractBundledPackageKeymap(keymapRelativePath) {
    ...
    const metadata = chevron.packages.packagesCache[packageName] || {};
  ...
  openKeybindingFile(keymapPath) {
    ...
    chevron.workspace.open(keymapPath);
  }
...
`;
    ...
    chevron.notifications.addInfo("Keybinding Copied");
