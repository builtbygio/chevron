...
const {Range, Point} = require('chevron')
...
class BracketMatcherView {
  constructor (editor, editorElement, matchManager) {
    ...
    this.subscriptions.add(
      ...
      chevron.commands.add(editorElement, 'bracket-matcher:go-to-matching-bracket', () =>
        this.goToMatchingBracket()
      ),

      chevron.commands.add(editorElement, 'bracket-matcher:go-to-enclosing-bracket', () =>
        this.gotoPrecedingStartBracket()
      ),

      chevron.commands.add(editorElement, 'bracket-matcher:select-inside-brackets', () =>
        this.selectInsideBrackets()
      ),

      chevron.commands.add(editorElement, 'bracket-matcher:close-tag', () =>
        this.closeTag()
      ),

      chevron.commands.add(editorElement, 'bracket-matcher:remove-matching-brackets', () =>
        this.removeMatchingBrackets()
      ),

      chevron.commands.add(editorElement, 'bracket-matcher:select-matching-brackets', () =>
  ...
  createMarker (bufferRange) {
    ...
    if (chevron.config.get('bracket-matcher.highlightMatchingLineNumber', {scope: this.editor.getRootScopeDescriptor()}) && this.gutter) {
