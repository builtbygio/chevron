const {CompositeDisposable, Point, Range} = require('chevron')
...
module.exports = class Whitespace {
  constructor () {
    ...
    this.subscriptions.add(chevron.commands.add('atom-workspace', {
      'whitespace:remove-trailing-whitespace': () => {
        let editor = chevron.workspace.getActiveTextEditor()
      ...
      'whitespace:save-with-trailing-whitespace': async () => {
        ...
        if (editor) {
          this.ignore = true
          editor.getPath() ? await editor.save() : chevron.workspace.getCenter().getActivePane().saveActiveItemAs()
          this.ignore = false
        }
      },
  ...
  handleEvents (editor) {
    ...
    let bufferSavedSubscription = buffer.onWillSave(() => {
      return buffer.transact(() => {
        ...
        if (chevron.config.get('whitespace.removeTrailingWhitespace', {
          scope: scopeDescriptor
        }) && !this.ignore) {
          this.removeTrailingWhitespace(editor, editor.getGrammar().scopeName)
        }

        if (chevron.config.get('whitespace.ensureSingleTrailingNewline', {scope: scopeDescriptor})) {
    ...
    let editorTextInsertedSubscription = editor.onDidInsertText(function (event) {
      ...
      if (chevron.config.get('whitespace.removeTrailingWhitespace', {
        ...
        if (!chevron.config.get('whitespace.ignoreWhitespaceOnlyLines', {
          scope: scopeDescriptor
        })) {
  ...
  removeTrailingWhitespace (editor, grammarScopeName) {
    ...
    const activeEditor = chevron.workspace.getActiveTextEditor()
    const cursorRows =
      activeEditor && activeEditor.getBuffer() === buffer
        ? new Set(activeEditor.getCursors().map(cursor => cursor.getBufferRow()))
        : new Set()

    const ignoreCurrentLine = chevron.config.get('whitespace.ignoreWhitespaceOnCurrentLine', {
      scope: scopeDescriptor
    })

    const ignoreWhitespaceOnlyLines = chevron.config.get('whitespace.ignoreWhitespaceOnlyLines', {
      scope: scopeDescriptor
    })

    const keepMarkdownLineBreakWhitespace =
      grammarScopeName === ('source.gfm' || 'text.md') &&
      chevron.config.get('whitespace.keepMarkdownLineBreakWhitespace')
  ...
  convertSpacesToTabs (editor) {
    ...
    let userTabSize = chevron.config.get('editor.tabLength', {
      scope: scope
    })
