...
module.exports = {
  ...
  load() {
    ...
    chevron.project.onDidChangePaths(() => this.scanProjectDirectories());
    return this.scanProjectDirectories();
  },

  scanProjectDirectories() {
    this.packageDirectories = [];
    return chevron.project.getDirectories().forEach(directory => {
  ...
  isEditingAnAtomPackageFile(editor) {
    ...
    for (var directory of Array.from(this.packageDirectories != null ? this.packageDirectories : [])) {
  ...
  getCompletions(line) {
    ...
    for (var completion of Array.from(propertyCompletions)) {
  ...
  getPropertyClass(name) {
    return __guard__(chevron[name] != null ? chevron[name].constructor : undefined, x => x.name);
  },
  ...
  loadProperty(propertyName, className, classes, parent) {
    ...
    for (var completion of Array.from(classCompletions)) {
      this.completions[propertyName].completions.push(completion);
      if (completion.type === 'property') {
        var propertyClass = this.getPropertyClass(completion.name);
...
var clone = function(obj) {
  const newObj = {};
  for (var k in obj) { var v = obj[k]; newObj[k] = v; }
  return newObj;
};
