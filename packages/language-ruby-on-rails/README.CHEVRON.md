# language-ruby-on-rails (Chevron)

TextMate-only Rails dialect overlays (`source.ruby.rails` plus html/js/sql/rjs).
Ships as JSON. 13c: no CSON in `grammars/` / `settings/` / `snippets/`.
