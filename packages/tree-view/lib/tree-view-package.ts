...
class TreeViewPackage {
  activate () {
    ...
    const showOnAttach = !chevron.workspace.getActivePaneItem()
    this.treeViewOpenPromise = chevron.workspace.open(treeView, {
      activatePane: showOnAttach,
      activateItem: showOnAttach
    })

    if (chevron.config.get("tree-view.hiddenOnStartup")) {
