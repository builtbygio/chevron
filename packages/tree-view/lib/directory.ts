...
class Directory {
  constructor ({name, fullPath, symlink, expansionState, isRoot, ignoredNames, useSyncFS, stats}) {
    ...
    if (chevron.config.get('tree-view.squashDirectoryNames') && !this.isRoot) {
  ...
  isPathIgnored (filePath) {
    if (chevron.config.get('tree-view.hideVcsIgnoredFiles')) {
      const repo = repoForPath(this.path)
      if (repo && repo.isProjectAtRoot() && repo.isPathIgnored(filePath)) return true
    }

    if (chevron.config.get('tree-view.hideIgnoredNames')) {
  ...
  getEntries () {
    ...
    for (let name of names) {
      ...
      // Prefer prototype Date getters / *Ms before _.pick drops them (Node 18+).
  ...
  sortEntries (combinedEntries) {
    if (chevron.config.get('tree-view.sortFoldersBeforeFiles')) {
