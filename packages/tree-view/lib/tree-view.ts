...
const TREE_VIEW_URI = 'chevron://tree-view';

const toggleConfig = keyPath => chevron.config.set(keyPath, !chevron.config.get(keyPath));
...
(TreeView = class TreeView {
  constructor(state) {
    ...
    process.nextTick(() => {
      ...
      this.disposables.add(chevron.styles.onDidAddStyleElement(onStylesheetsChanged));
      this.disposables.add(chevron.styles.onDidRemoveStyleElement(onStylesheetsChanged));
      return this.disposables.add(chevron.styles.onDidUpdateStyleElement(onStylesheetsChanged));
    });

    this.updateRoots(state.directoryExpansionStates);

    if ((state.selectedPaths != null ? state.selectedPaths.length : undefined) > 0) {
      for (var selectedPath of Array.from(state.selectedPaths)) { this.selectMultipleEntries(this.entryForPath(selectedPath)); }
    } else {
    ...
    this.disposables.add(this.onEntryMoved(({initialPath, newPath}) => {
      return (() => {
        const result = [];
        for (var editor of Array.from(chevron.workspace.getTextEditors())) {
          var filePath = editor.getPath();
  ...
  serialize() {
    return {
      directoryExpansionStates: new (function(roots) {
        for (var root of Array.from(roots)) { this[root.directory.path] = root.directory.serializeExpansionState(); }
  ...
  destroy() {
    for (var root of Array.from(this.roots)) { root.directory.destroy(); }
  ...
  getPreferredLocation() {
    if (chevron.config.get('tree-view.showOnRightSide')) {
      return 'right';
    } else {
      return 'left';
    }
  ...
  handleEvents() {
    ...
    [0, 1, 2, 3, 4, 5, 6, 7, 8].forEach(index => {
      return chevron.commands.add(this.element, `tree-view:open-selected-entry-in-pane-${index + 1}`, () => {
        return this.openSelectedEntryInPane(index);
      });
    });

    this.disposables.add(chevron.workspace.getCenter().onDidChangeActivePaneItem(() => {
      this.selectActiveFile();
      if (chevron.config.get('tree-view.autoReveal')) { return this.revealActiveFile({show: false, focus: false}); }
    })
    );
    this.disposables.add(chevron.project.onDidChangePaths(() => {
      return this.updateRoots();
    })
    );
    this.disposables.add(chevron.config.onDidChange('tree-view.hideVcsIgnoredFiles', () => {
      return this.updateRoots();
    })
    );
    this.disposables.add(chevron.config.onDidChange('tree-view.hideIgnoredNames', () => {
      return this.updateRoots();
    })
    );
    this.disposables.add(chevron.config.onDidChange('core.ignoredNames', () => {
      if (chevron.config.get('tree-view.hideIgnoredNames')) { return this.updateRoots(); }
    })
    );
    this.disposables.add(chevron.config.onDidChange('tree-view.sortFoldersBeforeFiles', () => {
      return this.updateRoots();
    })
    );
    return this.disposables.add(chevron.config.onDidChange('tree-view.squashDirectoryNames', () => {
      return this.updateRoots();
    })
    );
  }
  ...
  toggle() {
    return chevron.workspace.toggle(this);
  }

  show(focus) {
    return chevron.workspace.open(this, {
      searchAllPanes: true,
      activatePane: false,
      activateItem: false,
    }).then(() => {
      chevron.workspace.paneContainerForURI(this.getURI()).show();
      if (focus) { return this.focus(); }
    });
  }

  hide() {
    return chevron.workspace.hide(this);
  }
  ...
  unfocus() {
    return chevron.workspace.getCenter().activate();
  }
  ...
  fileViewEntryClicked(e) {
    ...
    const alwaysOpenExisting = chevron.config.get('tree-view.alwaysOpenExisting');
    if (detail === 1) {
      if (chevron.config.get('core.allowPendingPaneItems')) {
        const openPromise = chevron.workspace.open(filePath, {pending: true, activatePane: false, searchAllPanes: alwaysOpenExisting});
  ...
  openAfterPromise(uri, options) {
    ...
    if ((promise = this.currentlyOpening.get(uri))) {
      return promise.then(() => chevron.workspace.open(uri, options));
    } else {
      return chevron.workspace.open(uri, options);
    }
  }
  ...
  updateRoots(expansionStates) {
    ...
    const projectPaths = chevron.project.getPaths();
    if (projectPaths.length > 0) {
      ...
      this.roots = (() => {
        const result = [];
        for (var projectPath of Array.from(projectPaths)) {
          var left;
          ...
          var rawStats = stats;
          stats = _.pick(rawStats, ...Array.from(_.keys(rawStats)));
          for (var key of ["atime", "birthtime", "ctime", "mtime"]) {
            if ((rawStats[key] != null ? rawStats[key].getTime : undefined) != null) {
  ...
  getActivePath() { return __guardMethod__(chevron.workspace.getCenter().getActivePaneItem(), 'getPath', o => o.getPath()); }
  ...
  revealActiveFile(options) {
    ...
    if (!chevron.project.getPaths().length) { return Promise.resolve(); }

    let {show, focus} = options;

    if (focus == null) { focus = chevron.config.get('tree-view.focusOnReveal'); }
    const promise = show || focus ? this.show(focus) : Promise.resolve();
    return promise.then(() => {
      let activeFilePath;
      if (!(activeFilePath = this.getActivePath())) { return; }

      const [rootPath, relativePath] = Array.from(chevron.project.relativizePath(activeFilePath));
      ...
      return (() => {
        const result = [];
        for (var pathComponent of Array.from(activePathComponents)) {
          currentPath += path.sep + pathComponent;
          var entry = this.entryForPath(currentPath);
          if (entry.classList.contains('directory')) {
  ...
  copySelectedEntryPath(relativePath) {
    ...
    if (pathToCopy = this.selectedPath) {
      if (relativePath) { pathToCopy = chevron.project.relativize(pathToCopy); }
      return chevron.clipboard.write(pathToCopy);
    }
  }
  ...
  entryForPath(entryPath) {
    ...
    for (var entry of Array.from(this.list.querySelectorAll('.entry'))) {
  ...
  collapseDirectory(isRecursive, allDirectories) {
    ...
    if (allDirectories) {
      for (var root of Array.from(this.roots)) { root.collapse(true); }
      return;
    }
  ...
  openSelectedEntry(options, expandDirectory) {
    ...
    if (selectedEntry.classList.contains('directory')) {
      ...
      if (chevron.config.get('tree-view.alwaysOpenExisting')) {
  ...
  openSelectedEntrySplit(orientation, side) {
    ...
    const pane = chevron.workspace.getCenter().getActivePane();
    if (pane && selectedEntry.classList.contains('file')) {
      if (chevron.workspace.getCenter().getActivePaneItem()) {
        const split = pane.split(orientation, side);
        return chevron.workspace.openURIInPane(selectedEntry.getPath(), split);
      } else {
  ...
  openSelectedEntryInPane(index) {
    ...
    const pane = chevron.workspace.getCenter().getPanes()[index];
    if (pane && selectedEntry.classList.contains('file')) {
      return chevron.workspace.openURIInPane(selectedEntry.getPath(), pane);
    }
  }
  ...
  showSelectedEntryInFileManager() {
    ...
    if (!fs.existsSync(filePath)) {
      return chevron.notifications.addWarning(`Unable to show ${filePath} in ${this.getFileManagerName()}`);
    }

    return chevron.applicationDelegate.showItemInFolder(filePath);
  }

  showCurrentFileInFileManager() {
    let filePath;
    if (!(filePath = __guard__(chevron.workspace.getCenter().getActiveTextEditor(), x => x.getPath()))) { return; }
  ...
  openSelectedEntryInNewWindow() {
    ...
    if (pathToOpen = __guard__(this.selectedEntry(), x => x.getPath())) {
      return chevron.open({pathsToOpen: [pathToOpen], newWindow: true});
    }
  }
  ...
  removeSelectedEntries() {
    ...
    for (var root of Array.from(this.roots)) {
      var needle;
        ...
        var trashNext = index => {
          if (index >= selectedPaths.length) {
            ...
            if (failedDeletions.length > 0) {
              chevron.notifications.addError(this.formatTrashFailureMessage(failedDeletions), {
            ...
            if (chevron.config.get('tree-view.squashDirectoryNames')) { this.updateRoots(); }
            return;
          }
          ...
          return chevron.applicationDelegate.moveItemToTrash(selectedPath).then(ok => {
            let repo;
            if (ok) {
  ...
  pasteEntries() {
    ...
    return (() => {
      const result = [];
      for (var initialPath of Array.from(initialPaths)) {
  ...
  add(isCreatingFile) {
    ...
    dialog.onDidCreateFile(createdPath => {
      __guard__(this.entryForPath(createdPath), x => x.reload());
      chevron.workspace.open(createdPath);
      if (chevron.config.get('tree-view.squashDirectoryNames')) { this.updateRoots(); }
  ...
  removeProjectFolder(e) {
    ...
    if (pathToRemove != null) { return chevron.project.removePath(pathToRemove); }
  }
  ...
  deselect(elementsToDeselect) {
    ...
    for (var selected of Array.from(elementsToDeselect)) { selected.classList.remove('selected'); }
    return undefined;
  }
  ...
  copyEntry(initialPath, newDirectoryPath) {
    ...
    if (initialPathIsDirectory && realNewDirectoryPath.startsWith(realInitialPath)) {
      if (!fs.isSymbolicLinkSync(initialPath)) {
        chevron.notifications.addWarning('Cannot copy a folder into itself');
        return;
      }
    }
    ...
    try {
      ...
      return chevron.notifications.addWarning(`Failed to copy entry ${initialPath} to ${newDirectoryPath}`, {detail: error.message});
    }
  }
  ...
  moveEntry(initialPath, newDirectoryPath) {
    ...
    try {
      ...
      if (fs.isDirectorySync(initialPath) && realNewDirectoryPath.startsWith(realInitialPath)) {
        if (!fs.isSymbolicLinkSync(initialPath)) {
          chevron.notifications.addWarning('Cannot move a folder into itself');
          return;
        }
      }
    } catch (error1) {
      error = error1;
      chevron.notifications.addWarning(`Failed to move entry ${initialPath} to ${newDirectoryPath}`, {detail: error.message});
    }
  ...
  moveConflictingEntry(initialPath, newPath, newDirectoryPath) {
    try {
      ...
      this.emitter.emit('move-entry-failed', {initialPath, newPath});
      chevron.notifications.addWarning(`Failed to move entry ${initialPath} to ${newPath}`, {detail: error.message});
    }
  ...
  selectContinuousEntries(entry, deselectOthers) {
    ...
    if (parentContainer === currentSelectedEntry.parentElement) {
      ...
      for (var element of Array.from(elements)) { element.classList.add('selected'); }
    }
  ...
  onDragStart(e) {
    ...
    if (entry = e.target.closest('.entry')) {
      ...
      const initialPaths = [];
      for (var target of Array.from(this.getSelectedEntries())) {
  ...
  onDrop(e) {
    ...
    if (entry = e.target.closest('.entry.directory')) {
      ...
      return (() => {
        const result2 = [];
        for (entry of Array.from(e.dataTransfer.files)) {           result2.push(chevron.project.addPath(entry.path));
        }
        return result2;
      })();
    }
  ...
  isAtomTreeViewEvent(e) {
    for (var item of Array.from(e.dataTransfer.items)) {
