...
class GoBackView extends import_symbols_view.default {
  toggle() {
    ...
    const previousEditor = chevron.workspace.getTextEditors().find((e) => e.id === previousTag.editorId);
    if (previousEditor) {
      const pane = chevron.workspace.paneForItem(previousEditor);
      pane.setActiveItem(previousEditor);
      restorePosition();
    } else if (previousTag.file) {
      chevron.workspace.open(previousTag.file).then(restorePosition);
    }
  }
}
