...
const { TextEditor, Disposable, CompositeDisposable } = require('chevron');
...
class ProjectFindView {
  ...
  didShow() {
    ...
    this.tooltipSubscriptions = new CompositeDisposable(
      ...
      chevron.tooltips.add(this.refs.findAllButton, {
        title: "Find All",
  ...
  handleEvents() {
    ...
    let afterSearch = () => {
      if (chevron.config.get('find-and-replace.closeFindPanelAfterSearch')) {
  ...
  replaceAll() {
    ...
    return this.showResultPane().then(() => {
      ...
      const buttonChosen = chevron.confirm({
  ...
  findInCurrentlySelectedDirectory(selectedElement) {
    ...
    if (absolutePath) {
      let [rootPath, relativePath] = chevron.project.relativizePath(absolutePath);
      if (rootPath && chevron.project.getDirectories().length > 1) {
  ...
  showResultPane() {
    ...
    let openDirection = chevron.config.get('find-and-replace.projectSearchResultsPaneSplitDirection');
    if (openDirection !== 'none') { options.split = openDirection; }
    return chevron.workspace.open(ResultsPaneView.URI, options);
  }

  onFinishedReplacing(results) {
    if (!results.replacedPathCount) chevron.beep();
  ...
  setSelectionAsFindPattern() {
    const editor = chevron.workspace.getCenter().getActivePaneItem();
  ...
  updateSyntaxHighlighting() {
    if (this.model.getFindOptions().useRegex) {
      ...
      return this.replaceEditor.setGrammar(chevron.grammars.grammarForScopeName('source.js.regexp.replacement'));
    } else {
      this.findEditor.setGrammar(chevron.grammars.nullGrammar);
      return this.replaceEditor.setGrammar(chevron.grammars.nullGrammar);
    }
  }
