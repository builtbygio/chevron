...
const {TextBuffer, TextEditor, CompositeDisposable} = require('chevron');
...
class FindView {
  ...
  didHide() {
    ...
    let workspaceElement = chevron.views.getView(chevron.workspace);
  ...
  handleEvents() {
    ...
    this.subscriptions.add(chevron.commands.add(this.findEditor.element, {
      'core:confirm': () => this.confirm(),
      'find-and-replace:confirm': () => this.confirm(),
      'find-and-replace:show-previous': () => this.showPrevious()
    }));

    this.subscriptions.add(chevron.commands.add(this.replaceEditor.element, {
  ...
  focusFindEditor() {
    const activeEditor = chevron.workspace.getCenter().getActiveTextEditor()
  ...
  confirm() {
    this.findNext({focusEditorAfter: chevron.config.get('find-and-replace.focusEditorAfterSearch')});
  }

  showPrevious() {
    this.findPrevious({focusEditorAfter: chevron.config.get('find-and-replace.focusEditorAfterSearch')});
  }

  liveSearch() {
    let findPattern = this.findEditor.getText();
    if (findPattern.length === 0 || (findPattern.length >= chevron.config.get('find-and-replace.liveSearchMinimumCharacters') && !this.model.patternMatchesEmptyString(findPattern))) {
  ...
  markersUpdated(markers) {
    ...
    if (this.model.getFindOptions().findPattern) {
      ...
      if (this.findEditor.getElement().hasFocus() && results > 0 && chevron.config.get('find-and-replace.scrollToResultOnLiveSearch')) {
  ...
  updateSyntaxHighlighting() {
    if (this.model.getFindOptions().useRegex) {
      this.findEditor.setGrammar(chevron.grammars.grammarForScopeName('source.js.regexp'));
      this.replaceEditor.setGrammar(chevron.grammars.grammarForScopeName('source.js.regexp.replacement'));
    } else {
      this.findEditor.setGrammar(chevron.grammars.nullGrammar);
      this.replaceEditor.setGrammar(chevron.grammars.nullGrammar);
    }
  }
  ...
  updateFindEnablement() {
    ...
    if (editor && (!hadFindTooltip || isDisabled)) {
      ...
      this.findTooltipSubscriptions.add(chevron.tooltips.add(this.refs.nextButton, {
        title: "Find Next",
        keyBindingCommand: 'find-and-replace:find-next',
        keyBindingTarget: this.findEditor.element
      }));
      this.findTooltipSubscriptions.add(chevron.tooltips.add(this.refs.findAllButton, {
        title: "Find All",
  ...
  updateReplaceEnablement() {
    ...
    if (canReplace) {
      ...
      this.replaceTooltipSubscriptions.add(chevron.tooltips.add(this.refs.replaceNextButton, {
        title: "Replace Next",
        keyBindingCommand: 'find-and-replace:replace-next',
        keyBindingTarget: this.replaceEditor.element
      }));
      this.replaceTooltipSubscriptions.add(chevron.tooltips.add(this.refs.replaceAllButton, {
        title: "Replace All",
  ...
  showWrapIcon(icon) {
    if (!chevron.config.get('find-and-replace.showSearchWrapIcon')) return;
    let editor = this.model.getEditor();
    if (!editor) return;

    let editorView = chevron.views.getView(editor);
