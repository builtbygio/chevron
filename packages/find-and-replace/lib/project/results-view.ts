...
const { Range, CompositeDisposable, Disposable } = require('chevron');
...
class ResultsView {
  constructor({model}) {
    ...
    this.subscriptions = new CompositeDisposable(
      chevron.config.observe('editor.fontFamily', this.fontFamilyChanged.bind(this)),
  ...
  async confirmResult({pending} = {}) {
    ...
    if (selectedRow instanceof MatchRow) {
      ...
      const editor = await chevron.workspace.open(selectedRow.group.result.filePath, {
        pending,
        searchAllPanes: true,
        split: reverseDirections[chevron.config.get('find-and-replace.projectSearchResultsPaneSplitDirection')]
      })
  ...
  copyResult() {
    ...
    if (selectedRow.data.matches) {
      ...
      chevron.clipboard.write(selectedRow.data.matches[0].lineText);
    }
  }
  ...
  copyPath() {
    ...
    let [projectPath, relativePath] = chevron.project.relativizePath(filePath);
    if (projectPath && chevron.project.getDirectories().length > 1) {
      relativePath = path.join(path.basename(projectPath), relativePath);
    }
    chevron.clipboard.write(relativePath);
  }
  ...
  async openInNewTab() {
    if (this.selectedRowIndex !== -1) {
      ...
      if (result) {
        ...
        if (!exists) {
          editor = await chevron.workspace.open(filepath, {
                                             activatePane: false,
                                             activateItem: false
          });
        }
  ...
  toggleLeadingContextLines() {
    if (this.model.getFindOptions().leadingContextLineCount > 0) {
      ...
      const searchContextLineCountBefore = chevron.config.get('find-and-replace.searchContextLineCountBefore');
  ...
  toggleTrailingContextLines() {
    if (this.model.getFindOptions().trailingContextLineCount > 0) {
      ...
      const searchContextLineCountAfter = chevron.config.get('find-and-replace.searchContextLineCountAfter');
