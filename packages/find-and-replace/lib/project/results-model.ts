...
const {Emitter, TextEditor, Range} = require('chevron')
...
module.exports = class ResultsModel {
  constructor (findOptions, metricsReporter) {
    ...
    chevron.workspace.getCenter().observeActivePaneItem(item => {
      if (item instanceof TextEditor && chevron.project.contains(item.getPath())) {
  ...
  async search (findPattern, pathsPattern, replacePattern, options = {}) {
    ...
    const leadingContextLineCount = chevron.config.get('find-and-replace.searchContextLineCountBefore')
  ...
  replace (pathsPattern, replacePattern, replacementPaths) {
    ...
    const promise = chevron.workspace.replace(this.regex, replacePattern, replacementPaths, (result, error) => {
      if (result) {
