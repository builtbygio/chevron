...
const { Point, Range, Emitter, CompositeDisposable, TextBuffer } = require('chevron');
...
class BufferSearch {
  ...
  replace(markers, replacePattern) {
    ...
    const preserveCaseOnReplace = chevron.config.get('find-and-replace.preserveCaseOnReplace')
