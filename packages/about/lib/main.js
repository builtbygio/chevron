...
module.exports = {
  activate() {
    ...
    if (
      chevron.getReleaseChannel() === 'dev' ||
      (availableVersion && semver.lte(availableVersion, chevron.getVersion()))
    ) {
    ...
    this.subscriptions.add(
      chevron.commands.add('atom-workspace', 'about:clear-update-state', () => {
        this.clearUpdateState();
      })
    );
  },
  ...
  createModel() {
    ...
    this.model = new About({
      uri: AboutURI,
      currentAtomVersion: chevron.getVersion(),
  ...
  isUpdateAvailable() {
    let availableVersion = window.localStorage.getItem(AvailableUpdateVersion);
    return availableVersion && semver.gt(availableVersion, chevron.getVersion());
  },
