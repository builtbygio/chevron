...
(WrapGuideElement = class WrapGuideElement {
  ...
  handleEvents() {
    ...
    this.subscriptions.add(chevron.config.onDidChange('editor.fontSize', () => {
  ...
  handleConfigEvents() {
    ...
    const updatePreferredLineLengthCallback = args => {
      ...
      let columns = chevron.config.get('wrap-guide.columns', {scope: this.editor.getRootScopeDescriptor()});
      if (columns.length > 0) {
        columns[columns.length - 1] = args.newValue;
        columns = uniqueAscending(Array.from(columns).filter((i) => i <= args.newValue));
        chevron.config.set('wrap-guide.columns', columns,
          {scopeSelector: `.${this.editor.getGrammar().scopeName}`});
      }
      return this.updateGuide();
    };
    this.configSubscriptions.add(chevron.config.onDidChange(
    ...
    const updateGuidesCallback = args => {
      ...
      if (columns != null ? columns.length : undefined) {
        chevron.config.set('wrap-guide.columns', columns);
        chevron.config.set('editor.preferredLineLength', columns[columns.length - 1],
          {scopeSelector: `.${this.editor.getGrammar().scopeName}`});
        return this.updateGuide();
      }
    };
    return this.configSubscriptions.add(chevron.config.onDidChange(
      'wrap-guide.columns',
      {scope: this.editor.getRootScopeDescriptor()},
  ...
  getDefaultColumn() {
    return chevron.config.get('editor.preferredLineLength', {scope: this.editor.getRootScopeDescriptor()});
  }

  getGuidesColumns(path, scopeName) {
    let left;
    const columns = (left = chevron.config.get('wrap-guide.columns', {scope: this.editor.getRootScopeDescriptor()})) != null ? left : [];
  ...
  isEnabled() {
    let left;
    return (left = chevron.config.get('wrap-guide.enabled', {scope: this.editor.getRootScopeDescriptor()})) != null ? left : true;
  }
  ...
  appendGuides() {
    ...
    return (() => {
      const result = [];
      for (var column of Array.from(columns)) {
