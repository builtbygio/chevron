...
module.exports = {
  activate () {
    ...
    this.updateTraversalKeybinds = () => {
      ...
      let bindings = chevron.keymaps.findKeyBindings({
      ...
      bindings = chevron.keymaps.findKeyBindings({
        target: document.body,
      ...
      if (chevron.config.get(enableMruConfigKey)) {
        chevron.keymaps.removeBindingsFromSource(keyBindSource)
      } else {
        ...
        chevron.keymaps.add(keyBindSource, disabledBindings, 0)
      }
    }

    this.subscriptions.add(chevron.config.observe(enableMruConfigKey, () => this.updateTraversalKeybinds()))
    this.subscriptions.add(chevron.keymaps.onDidLoadUserKeymap(() => this.updateTraversalKeybinds()))
    ...
    const paneContainers = {
      center: chevron.workspace.getCenter(),
      left: chevron.workspace.getLeftDock(),
      right: chevron.workspace.getRightDock(),
      bottom: chevron.workspace.getBottomDock()
    }
