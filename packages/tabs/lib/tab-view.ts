...
(TabView = class TabView {
  ...
  handleEvents() {
    ...
    if (typeof this.item.onDidSave === 'function') {
      const onDidSaveDisposable = this.item.onDidSave(event => {
        ...
        if (event.path !== this.path) {
          ...
          if (chevron.config.get('tabs.enableVcsColoring')) { return this.setupVcsStatus(); }
        }
      });
    ...
    this.subscriptions.add(chevron.config.observe('tabs.showIcons', () => {
      return this.updateIconVisibility();
    })
    );

    return this.subscriptions.add(chevron.config.observe('tabs.enableVcsColoring', isEnabled => {
  ...
  updateTooltip() {
    ...
    if (this.path) {
      return this.tooltip = chevron.tooltips.add(this.element, {
        title: this.path,
        html: false,
        delay: {
          show: 1000,
  ...
  updateIconVisibility() {
    if (chevron.config.get('tabs.showIcons')) {
  ...
  repoForPath() {
    for (var dir of Array.from(chevron.project.getDirectories())) {
      if (dir.contains(this.path)) { return chevron.project.repositoryForDirectory(dir); }
    }
  ...
  updateVcsColoring() {
    ...
    if (this.status && chevron.config.get('tabs.enableVcsColoring')) {
