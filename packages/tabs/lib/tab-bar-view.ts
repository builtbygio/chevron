...
(TabBarView = class TabBarView {
  constructor(pane, location) {
    ...
    this.subscriptions.add(chevron.commands.add(this.paneElement, {
    ...
    const addElementCommands = commands => {
      ...
      return this.subscriptions.add(chevron.commands.add(this.element, commandsWithPropagationStopped));
    };
    ...
    this.subscriptions.add(chevron.config.observe('tabs.tabScrolling', value => this.updateTabScrolling(value)));
    this.subscriptions.add(chevron.config.observe('tabs.tabScrollingThreshold', value => this.updateTabScrollingThreshold(value)));
    this.subscriptions.add(chevron.config.observe('tabs.alwaysShowTabBar', () => this.updateTabBarVisibility()));
  ...
  addTabForItem(item, index) {
    ...
    if (chevron.config.get('tabs.addNewTabsAtEnd')) {
  ...
  updateTabBarVisibility() {
    ...
    if (chevron.config.get('tabs.alwaysShowTabBar') || (this.pane.getItems().length > 1)) {
  ...
  openInNewWindow(tab) {
    ...
    const pathsToOpen = [chevron.project.getPaths(), itemURI].reduce(((a, b) => a.concat(b)), []);
    return chevron.open({pathsToOpen, newWindow: true, devMode: chevron.devMode, safeMode: chevron.safeMode});
  }
  ...
  getWindowId() {
    return this.windowId != null ? this.windowId : (this.windowId = chevron.getCurrentWindow().id);
  }
  ...
  onDrop(event) {
    ...
    if (fromWindowId === this.getWindowId()) {
      ...
      chevron.workspace.open(droppedURI).then(item => {
        // Move the item from the pane it was opened on to the target pane
        // where it was dropped onto
        const activePane = chevron.workspace.getActivePane();
  ...
  onPaneDragEnter(event) {
    if (!this.isAtomTabEvent(event)) { return; }
    if (!this.itemIsAllowed(event, this.location)) { return; }
    if ((this.pane.getItems().length > 1) || chevron.config.get('tabs.alwaysShowTabBar')) { return; }
  ...
  onDoubleClick(event) {
    ...
    if (tab = this.tabForElement(event.target)) {
      ...
      chevron.commands.dispatch(this.element, 'application:new-file');
      return event.preventDefault();
    }
  }
  ...
  removeDropTargetClasses() {
    let dropTarget;
    const workspaceElement = chevron.workspace.getElement();
