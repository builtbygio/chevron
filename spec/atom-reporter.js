...
const formatStackTrace = function(spec, message, stackTrace) {
  ...
  let lines = [];
  for (var line of Array.from(stackTrace.split('\n'))) {
...
`;
      ...
      for (var element of Array.from(this.element.querySelectorAll('[outlet]'))) {
    ...
    specTitle(spec) {
      ...
      let suiteString = "";
      let indent = "";
      for (var desc of Array.from(parentDescs)) {
    ...
    addSpecs(specs) {
      ...
      for (var spec of Array.from(specs)) {
...
`;
    ...
    for (var result of Array.from(this.spec.results().getItems())) {
      if (!result.passed()) {
        var stackTrace = formatStackTrace(this.spec, result.message, result.trace.stack);
        ...
        if (stackTrace) {
          var traceElement = document.createElement('pre');
