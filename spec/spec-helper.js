...
var ensureNoDeprecatedFunctionCalls = function() {
  ...
  if (deprecations.length > 0) {
    ...
    Error.prepareStackTrace = function(error, stack) {
      const output = [];
      for (var deprecation of Array.from(deprecations)) {
        ...
        for (stack of Array.from(deprecation.getStacks())) {
          for (var {functionName, location} of Array.from(stack)) {
...
var ensureNoDeprecatedStylesheets = function() {
  ...
  return (() => {
    const result = [];
    for (var sourcePath in deprecations) {
      var deprecation = deprecations[sourcePath];
      var title =
...
window.advanceClock = function(delta) {
  ...
  return (() => {
    const result = [];
    for (var callback of Array.from(callbacks)) {       result.push(callback());
    }
    return result;
  })();
};
