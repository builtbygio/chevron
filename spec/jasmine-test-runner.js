...
module.exports = function({logFile, headless, testPaths, buildAtomEnvironment}) {
  ...
  if (process.env.APM_TEST_PACKAGES) {
    ...
    for (var packName of Array.from(testPackages)) {
      var userPack = path.join(userHome, 'packages', packName);
      var loadablePack = path.join(atomHome, 'packages', packName);
  ...
  for (var testPath of Array.from(testPaths)) { requireSpecs(testPath); }
