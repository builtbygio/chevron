...
Authoritative catalog: [language-stack.md](./language-stack.md). 19 packages ship a tree-sitter grammar (18 also keep TextMate; rust is tree-sitter only). 14 are TextMate-only; `language-source` is settings only. 13b first tranche done; later tranche started (less, scss). Remaining later: perl, clojure, csharp. Indented Sass stays TextMate. “keep TextMate” is a valid owner decision — that list is why first-mate stays.
...
- **Status:** first tranche landed; later tranche started. **this change** (SCSS, stacked on Less). Remaining later: perl, clojure, csharp
