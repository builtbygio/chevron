...
**Related:** [cpm-design-eli5.md](../orientation/cpm-design-eli5.md), [cpm-cutover.md](../orientation/cpm-cutover.md), [REBRANDING.md](../decisions/REBRANDING.md)  
...
cpm is **not** a rewrite of the editor’s package loader (`PackageManager`, `global.chevron`). It is the **installer and rebuild CLI** (and later, registry client). Product policy: [REBRANDING.md](../decisions/REBRANDING.md), [chevron-architecture-modernization.md](./chevron-architecture-modernization.md).
...
Locked product policy ([REBRANDING.md](../decisions/REBRANDING.md), [chevron-architecture-modernization.md](./chevron-architecture-modernization.md)):
...
> **The Pulsar registry is a client implementation, not a product store.** cpm can resolve a name against it; that is not a promise that community install is open. The catalog stays **owned-only** until package host v2 ships as the T2 isolation model ([package-ecosystem-strategy.md](../decisions/package-ecosystem-strategy.md), [package host design](./security-phase-s-package-host.md)). When T2 reopens, community packages run **in the host** — that is what makes reopening defensible, not a registry URL. Do not write product copy that implies a store.
...
- Author docs: [cpm-prebuilds.md](../orientation/cpm-prebuilds.md); example workflow `.github/workflows/cpm-prebuild-example.yml`.
...
Exact version numbers for a named release (e.g. 0.4.0) are editorial; the roadmap itself is closed. Cutover notes: [cpm-cutover.md](../orientation/cpm-cutover.md).
...
docs/reference/cpm-design.md            # this file (authoritative)
docs/orientation/cpm-design-eli5.md       # plain-language companion
```
...
5. **~~Bundled `packageDependencies` at build time~~ — RESOLVED Option A (2026-07-21):** all 91 entries are also root `dependencies` (`file:` + git pins). Host npm install of the app tree is sufficient; `packageDependencies` remains a metadata map for runtime/build. Spike: [cpm-phase-0-spike.md](../process/cpm-phase-0-spike.md).  
