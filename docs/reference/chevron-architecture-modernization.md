...
| Editor `sandbox: false` **intentional** (Option C) | `docs/decisions/security-phase-s-decision.md`; `src/preload-natives.js` `phaseSDecision` |
| No `@electron/remote`; `src/remote-compat.js` + `register-renderer-ipc.js` | `static/preload.js` 32–35; `docs/reference/remote-ipc-inventory.md` |
...
| cpm Phases 0–4; the `apm` name is retired | `docs/reference/cpm-design.md`; `cpm/`; `script/ci/no-apm.test.js` |
| Chevron-only API policy | `docs/decisions/REBRANDING.md`; `require('atom')` is `MODULE_NOT_FOUND` (`exports/atom.js` is gone) |
| Owned catalog only until host v2 | `docs/decisions/package-ecosystem-strategy.md` |
...
| LSP phases 0–5 shipped (utilityProcess host + workspace trust) | `docs/reference/lsp-design.md`; `src/lsp/`; `src/main-process/workers/lsp-host.js` |
| Custom V8 snapshot on Linux/Windows; Darwin stock | `script/lib/packaging-policy.js` `darwin-boot-crash`; #121/#125 |
| 0 `atom/*` app git pins | #79; `docs/reference/package-ownership-inventory.md` |

`docs/reference/atom-architecture.md` is **stale** on several of these (still describes apm Node 12, `out/Atom.app`, community packages under `~/.atom`). Do not treat it as the current model. This document supersedes it as the architecture target.
...
| Pain | Fact |
|------|------|
| Cold start on old Mac | ~7.8 s median wall; 3 s in `setup-window` → `initialize` (`docs/reference/startup-snapshot-plan.md` §4.1) |
...
| Test split is inverted | Fast `node --test script/ci/*` is the PR gate; `script/test` Jasmine+Mocha is nightly, not a merge gate, and first nightlies are measurement (`docs/reference/jasmine-ci.md`) |
...
| `docs/reference/atom-architecture.md`, `cpm-design.md` “dual-support forever” | docs | **Delete** the teaching — **PR 1 (this change)** |
...
| N2 | Flip editor `sandbox: true` | Option C is the end state for *this* architecture (`docs/decisions/security-phase-s-decision.md`). Security is T2 restrict + guests + IPC + utilityProcess + fuses. |
...
| N6 | Open community install before host v2 | `docs/decisions/package-ecosystem-strategy.md`. cpm/Pulsar client may stay in-tree; product UX must not promise a store. |
...
- LSP (`src/lsp/`) is the semantic layer and must stay **out of the renderer** (G1 in `docs/reference/lsp-design.md`).
...
- **Owned git pins still ship CSON.** Counted 2026-08-15: **70** `.cson` files at pin depth (not nested `node_modules`). Almost all `language-*`:
  ...
  Plus ~38 of those are `grammars/*.cson`. `docs/orientation/owned-package-modernization-checklist.md` already lists “CSON → JSON for keymaps/menus/grammars/snippets” as **iterative per-package** work.
...
- Phase S explicitly **deferred** S4 sendSync→invoke (`docs/decisions/security-phase-s-decision.md` exit table).
...
2. **S4 is inventory + one slice per PR**, not “migrate non-boot sendSync” in a single merge. `docs/reference/remote-ipc-inventory.md` §11 is the row list:
...
5. **Package host v2** remains the *later* isolation model. H1 does not implement it. H3 is an **epic** that implements the already-written `docs/reference/security-phase-s-package-host.md` in several PRs after owner sign-off.
...
4. **Signing / notarization** is a release-engineering track, not architecture. When it happens, Squirrel feed may return; do not point it at unsigned bits (`docs/reference/releases.md`).
...
| Full editor | `script/test` → packaged binary → Jasmine renderer + Mocha main | Linux nightly + `jasmine` label (`docs/reference/jasmine-ci.md`) |
...
| # | Decision | Rationale |
|---|----------|-----------|
| D1 | **Stay on Electron. Do not rebase Pulsar. Do not flip `sandbox: true`.** | Electron 43 + Option C is a finished security/product decision. Pulsar is a compatibility fork. Sandbox:true without a buffer-host is dishonest (`docs/decisions/security-phase-s-decision.md`). |
| D2 | **Chevron-only. Atom surfaces are shims to delete, not a platform.** | Owner policy (`docs/decisions/REBRANDING.md`). Dual-support is how you never delete Task/CSON/`require('atom')`. |
...
| D11 | **`remote-compat` shrinks one slice at a time. New IPC is `invoke` on allowlisted channels.** S4 H1 is inventory + one slice (not FS, not workers). | S4 was deferred from Phase S; it is now a modernization item, not a security gate. `docs/reference/remote-ipc-inventory.md` §11 is the row list. |
...
| **Cons** | Declined with a written reason: superstring/tree-sitter/oniguruma are `renderer-hot`; no time-boxed spike proved acceptable latency (`docs/decisions/security-phase-s-decision.md`). Doing it “as modernization” would be checking a box while lying about `.node` loads. |
...
| Workspace trust | LSP servers still gated (`docs/reference/lsp-design.md` §6). Slimming `github` reduces Relay/GraphQL attack surface. |
...
| Tests | `unit-and-cpm` duration; Jasmine nightly JUnit artifact (`docs/reference/jasmine-ci.md`). Treat nightly redness as a dashboard, not a page. |
...
3. No silent installer (`docs/reference/releases.md`). Users opt into new builds.
...
| `docs/decisions/REBRANDING.md` | Chevron-only surfaces |
| `docs/decisions/package-ecosystem-strategy.md` | Owned catalog; host v2 later |
| `docs/decisions/security-phase-s-decision.md` | Option C |
| `docs/reference/security-phase-s-package-host.md` | Host v1/v2 |
| `docs/reference/security-threat-model.md` | Trust tiers |
| `docs/reference/lsp-design.md` | Language-server architecture (shipped) |
| `docs/reference/language-stack.md` | Tree-sitter vs TextMate catalog + exception list (PR 13) |
| `docs/reference/cpm-design.md` | Installer (shipped; dual-support text is stale) |
| `docs/reference/startup-snapshot-plan.md` | Snapshot measurements |
| `docs/reference/packaging.md` | Packager 15 + unpack + snapshot policy |
...
- **Files:** `docs/reference/chevron-architecture-modernization.md` (this document), `docs/reference/atom-architecture.md` (rewrite to match this target + current process model), `GROK.md` (pointer), `docs/reference/cpm-design.md` (strike “dual-support forever”), `docs/reference/package-node-policy.md` (stop recommending `Task`; Chevron-only), `docs/reference/build-modernization.md` (snapshot status: Linux/Windows on, Darwin stock), **`exports/chevron.js` header** (stop saying `require('atom')` exists “so community packages keep working”; keep the shim), `exports/atom.js` comment only
- **Depends on:** none
...
- **Status:** **this change**
- **Files:** `script/package.json`, `script/lib/package-application.js`, `script/lib/packaging-policy.js`, `script/lib/include-path-in-packaged-app.js`, `script/ci/find-packaged-app.test.js`, `script/ci/packaging-policy.test.js`, `docs/reference/packaging.md`
...
- **Status:** **this change**
- **Files:** `docs/reference/remote-ipc-inventory.md` §11, `exports/remote.js`, `src/application-delegate.js`, `src/renderer-ipc.js`, `src/main-process/register-renderer-ipc.js`, `src/atom-environment.js`, `src/title-bar.js`
...
- **Status:** **this change**
- **Files:** `docs/reference/jasmine-ci.md`, `CONTRIBUTING.md`, `script/package.json`, `script/build` Coffee/Babel no-op entry points. **Not** `transpile-cson-paths.js`. **Not** coffeelint/donna/tello (still invoked)
- **Depends on:** none
...
- **Status:** landed (#144)
- **Files:** `src/compile-cache.js`, `src/coffee-script.js`, `src/babel.js`, `src/module-cache.js`, `docs/process/babel-coffee-isolation-plan.md`, `script/ci/legacy-transpile.test.js`
...
- **Status:** landed (#145)
- **Files:** `docs/reference/startup-snapshot-plan.md`, `script/ci/measure-startup.js`, `.github/workflows/ci.yml` (Windows measure step), CHANGELOG, `GROK.md`
- **Depends on:** none
...
- **Status:** landed (#146)
- **Files:** new `docs/reference/language-stack.md`, `src/grammar-registry.js` (comments/metrics), `script/ci/language-stack.test.js`
- **Depends on:** PR 1
...
- **Status:** started. Done: `language-source`, `language-hyperlink`, `language-text`, `language-todo`, `language-gfm`, `language-less`, `language-make`, `language-mustache`, `language-sql`, `language-toml`, `language-yaml`, `language-clojure`, `language-coffee-script`, `language-perl`, `language-php`, `language-property-list`, `language-xml`, `language-csharp`, `language-git`, `language-objective-c`, `language-sass`, `language-ruby-on-rails`. Remaining inventory in [language-stack.md](./language-stack.md) §13c is empty. `season` stays (PR 5b later).
...
Not one PR. Implements `docs/reference/security-phase-s-package-host.md`. **Owner sign-off given 2026-08-17**; slices 21.1–21.5 built as a stack. Slices:
...
- **Files:** `script/lib/generate-metadata.js` 12–20, Windows install/migrate notes, `docs/reference/releases.md`
- **Status:** **done (2026-08-18).** `generate-metadata.js` writes `chevron` / `chevron-<channel>`. **No migration was built** — the owner confirmed there is no Windows install base, so there is no userData to orphan. See [windows-userdata-migrate.md](../decisions/windows-userdata-migrate.md).
- **Depends on:** ~~explicit migrate plan for existing `AppData\Local\atom` trees~~ — resolved: no Windows users, nothing to migrate. **Not** H1 packaging (PR 6)
...
- **Files:** `script/lib/code-sign-on-mac.js`, Windows signtool, `docs/reference/releases.md`
